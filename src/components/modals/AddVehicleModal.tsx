...
export function AddVehicleModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const save = () => {
    const id =
      (document.getElementById('avId') as HTMLInputElement).value.trim() ||
      `TRK-${Math.floor(100 + Math.random() * 900)}`;
    const driver =
      (document.getElementById('avDriver') as HTMLInputElement).value.trim() || 'New Driver';
    ...
    const fuel = Math.min(
      ...
      Math.max(
        5,
        parseInt(
          (document.getElementById('avFuel') as HTMLInputElement).value || '80',
          10
        )
      )
    );
    const cargo =
      (document.getElementById('avCargo') as HTMLInputElement).value.trim() || 'N/A';
    const routeId = routeSel === 'none' ? null : routeSel;

    let pos = depot;
    let status: 'En Route' | 'Idle' | 'At Depot' | 'Maintenance' = 'Idle';
    let speed = 0;
    let i = 0;
    if (routeId) {
    ...
    addVehicle({
      ...
      fuel,
      speed,
      odo: Math.floor(5000 + Math.random() * 200000),
      pos,
      routeId,
      i,
      cargo,
  ...
  return (
    <Modal open={open} onClose={onClose} maxWidth="max-w-md sm:max-w-xl">
      <div className="flex items-start justify-between">
        ...
        <button className="chip badge" onClick={onClose}>
          Close
        </button>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <input id="avId" className="input" placeholder="TRK-201" />
        </div>
        <div>
          <div className="label">Driver</div>
          <input id="avDriver" className="input" placeholder="Thandi" />
        </div>
        <div>
        ...
        <div className="sm:col-span-2">
          ...
          <input id="avCargo" className="input" placeholder="FMCG" />
        </div>
      </div>
